  ...
  for (var j = i + 1; j < metrics.length; j++) {
    ...
    if (i + 1 <= metrics.length) {
      for (var k = 0; k < metrics.length; k++) {
        x = metrics[j][k] - metrics[i][k] / metrics[i][i] * multivar; //console.log(metrics[j][k],metrics[i][k],metrics[i][i],multivar);

        metrics[j][k] = x;
      }

      y = metans[j][0] - metans[i][0] / metrics[i][i] * multivar; //console.log(metans[j][0],metans[i][0],metrics[i][i],multivar);
