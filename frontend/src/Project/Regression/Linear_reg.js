...
var X = [];
var Y = [];
export function LinearReg_receivedata(pdata) {
  // data = pdata;
  X = pdata.metrics;
  Y = pdata.metans;
}
class LinearReg extends Component {
  constructor() {
    super();
    ...
    if (X.length === 0) {
      for (let i = 0; i < 1; i++) {
        ...
        for (let j = 0; j < val; j++) {
          ...
          X[i].push(inputVal);
          if (i === 0) {
            var inpuanstVal = document.getElementById("Y" + i + "0").value;
  ...
  Createinput = () => {
    ...
    for (let i = 0; i < 1; i++) {
      ...
      for (let j = 0; j < val; j++) {
        ...
        retx += ";/> ";
        if (i === 0) {
          rety += ' <input type="number" id="Y' + i + '0" style="width:20px;" ';
          if (Y.length !== 0) {
            rety += ' value = "' + Y[i][j] + '"';
          }
          rety += " />";
